...
public static class Extensions
{
	...
	public static void Set_To_Mouse_Position(this GameObject go)
	{
		Vector3 currentScreenPosition = new Vector3(Input.mousePosition.x, Input.mousePosition.y);
		Vector3 currentPos = Camera.main.ScreenToWorldPoint(currentScreenPosition);
		go.SetPositionXY(currentPos.x, currentPos.y);
	}

	public static void Set_Position_To(this GameObject go, GameObject target)
	{
		go.SetPositionXY(target.GetPositionX(), target.GetPositionY());
	}
